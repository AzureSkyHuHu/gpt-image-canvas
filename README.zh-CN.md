...
鉴权默认开启。实时生成前，请在 `.env` 中设置 `OPENAI_API_KEY`，或通过管理员面板给访问 token 绑定上游 key。应用默认使用官方 OpenAI Image API 和 `gpt-image-2`。如需转发到 OpenAI 兼容端点，请在 `.env` 中设置 `OPENAI_BASE_URL`；如需使用不同的兼容图像模型，请设置 `OPENAI_IMAGE_MODEL`。
