...
后端默认按当前访问 token 限制最多保存 `100` 张本地图片，可通过 `.env` 调整：

```env
APP_MAX_ASSETS_PER_TOKEN=100
```

设置为 `0` 可关闭限制。达到上限后，文生图和参考图生成接口会返回 `429`：

```json
{
  "error": {
    "code": "asset_limit_exceeded",
    "message": "当前访问 token 已保存 100 张图片，最多允许 100 张；本次请求需要 1 张，剩余额度 0 张。请先删除旧图片后再生成。"
  }
}
```

删除 Gallery 图片或彻底删除生成历史并清理图片文件后，会释放对应额度。
