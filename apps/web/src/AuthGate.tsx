...
export function AuthGate({ children }: AuthGateProps) {
  ...
  const sortedTokens = useMemo(() => [...tokens].sort((a, b) => b.createdAt.localeCompare(a.createdAt)), [tokens]);
  ...
  function moveSessionBarDrag(event: PointerEvent<HTMLButtonElement>): void {
    ...
    if (!drag.dragging && Math.hypot(deltaX, deltaY) < SESSION_BAR_DRAG_THRESHOLD) return;

    drag.dragging = true;
    event.preventDefault();
    updateSessionBarPosition(
      clampSessionBarPosition({ left: drag.startLeft + deltaX, bottom: drag.startBottom - deltaY }, drag.width, drag.height)
    );
  }
    ...
    <div className="auth-shell">
      <section className="auth-panel" aria-labelledby="auth-title">
        ...
        <form className="auth-form" onSubmit={(event) => void submitAccessToken(event)}>
          <label>
            <span>访问 token</span>
            <input value={tokenInput} onChange={(event) => setTokenInput(event.target.value)} autoComplete="off" spellCheck={false} />
          </label>
              ...
              <div className="auth-admin-workspace">
                ...
                {createdToken ? (
                  <div className="auth-created-token">
                    ...
                    <button type="button" onClick={() => setShowCreatedToken((value) => !value)}>
                      {showCreatedToken ? <EyeOff size={16} /> : <Eye size={16} />}
                    </button>
                    <button type="button" onClick={() => void navigator.clipboard.writeText(createdToken)}>
                      <Copy size={16} />
                    </button>
                  </div>
                ) : null}
                ...
                <form className="auth-token-form" onSubmit={(event) => void submitTokenForm(event)}>
                  <label>
                    <span>名称</span>
                    <input value={tokenForm.label} onChange={(event) => updateTokenForm("label", event.target.value)} />
                  </label>
                  <label>
                    <span>{isEditing ? "替换访问 token" : "访问 token"}</span>
                    <input value={tokenForm.accessToken} onChange={(event) => updateTokenForm("accessToken", event.target.value)} />
                  </label>
                  <label>
                    <span>{isEditing ? "替换上游 key" : "上游 API key"}</span>
                    <input value={tokenForm.upstreamApiKey} onChange={(event) => updateTokenForm("upstreamApiKey", event.target.value)} />
                  </label>
                  <label>
                    <span>上游 Base URL</span>
                    <input value={tokenForm.upstreamBaseURL} onChange={(event) => updateTokenForm("upstreamBaseURL", event.target.value)} />
                  </label>
                  <label>
                    <span>模型</span>
                    <input value={tokenForm.upstreamModel} onChange={(event) => updateTokenForm("upstreamModel", event.target.value)} />
                  </label>
                  <label className="auth-check">
                    <input type="checkbox" checked={tokenForm.enabled} onChange={(event) => updateTokenForm("enabled", event.target.checked)} />
                    <span>启用</span>
                  </label>
                  {tokenError ? <p className="auth-form__error auth-token-form__wide">{tokenError}</p> : null}
                  <div className="auth-token-form__actions">
                    {isEditing ? <button type="button" onClick={resetTokenForm}>取消编辑</button> : null}
                    ...
                    sortedTokens.map((item) => (
                      <article className="auth-token-item" key={item.id}>
                        <div>
                          ...
                          <dl>
                            <div><dt>访问</dt><dd>{item.tokenPreview}</dd></div>
                            <div><dt>上游</dt><dd>{item.upstreamApiKeyPreview}</dd></div>
                            <div><dt>Base</dt><dd>{item.upstreamBaseURL || "默认"}</dd></div>
                            <div><dt>模型</dt><dd>{item.upstreamModel || "默认"}</dd></div>
                          </dl>
                        </div>
                        <div className="auth-token-item__actions">
                          <button type="button" onClick={() => editToken(item)}>编辑</button>
                          <button type="button" onClick={() => void deleteToken(item.id)}>
                            <Trash2 size={15} />
                          </button>
                        </div>
                      </article>
...
function writeSessionHint(): void {
  try {
    ...
    // The server cookie remains the source of truth.
  }
}
...
function readSessionBarPosition(): SessionBarPosition | undefined {
  try {
    ...
    if (!Number.isFinite(parsed.left) || !Number.isFinite(parsed.bottom)) return undefined;
    return clampSessionBarPosition({ left: Number(parsed.left), bottom: Number(parsed.bottom) });
  } catch {
    return undefined;
  }
}
...
function saveSessionBarPosition(position: SessionBarPosition): void {
  try {
    ...
    // Dragging still works for the current page.
  }
}
