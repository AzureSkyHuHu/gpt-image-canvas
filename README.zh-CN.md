...
- 新增凭据感知路由：API Key 优先，无 API Key 时可使用 Codex 登录；两者都没有时展示专业首页。
...
- 用于实时生成的 OpenAI API key，或在应用中完成的 Codex 登录。没有凭证时应用仍可启动，并会停留在首页直到有可用生成服务。
...
# 填写 .env 中的 APP_ADMIN_PASSWORD 和 APP_SESSION_SECRET；本地临时单人开发可把 APP_AUTH_ENABLED 改成 false。
pnpm dev
```
...
鉴权默认开启。公网部署应要求访问 token 登录，并通过管理员面板给每个访问 token 绑定上游 API key。应用默认使用官方 OpenAI Image API 和 `gpt-image-2`。如需转发到 OpenAI 兼容端点，请在 `.env` 中设置 `OPENAI_BASE_URL`；如需使用不同的兼容图像模型，请设置 `OPENAI_IMAGE_MODEL`。

打开 `http://localhost:5173` 使用 Web 应用。

## 认证与路由

- `APP_AUTH_ENABLED=true` 时，访问 token 登录仍是应用访问模型。每个 token 拥有独立的项目、Gallery、COS 配置、生成历史和图片额度。
- 普通访问 token 用户只使用该 token 绑定的上游 API key 生成，不会使用 Codex fallback。
- 管理员/本机可以登录 Codex，把 Codex 作为全局兜底 provider。管理员登录后进入本机工作区（`Admin local`），但不会让普通访问 token 用户获得 Codex fallback。本地单人模式（`APP_AUTH_ENABLED=false`）下，provider 优先级是 `OPENAI_API_KEY`、Codex 会话、`missing_provider`。
- `/` 是本地/无 provider 状态下的凭据感知首页，`/canvas` 是画布工作区，`/gallery` 在通过访问控制后可用。
- `接入 API` 弹窗只展示 `.env` 配置说明，不会在浏览器中保存、提交或回显 API Key。
- `Codex 登录` 弹窗会启动 Codex 设备登录，展示验证链接和用户码；授权后的 OAuth token 只保存在本地 API 侧。
...
如果宿主机 Node.js 版本低于 Vite 要求，或可选原生依赖是在另一个环境里安装的，可以改用本机 Docker Node 镜像验证构建：
...
这个命令只作为验证兜底；不要提交生成的构建输出或 `node_modules`。
...
```powershell
Copy-Item .env.example .env
# 填写 .env 中的 APP_ADMIN_PASSWORD 和 APP_SESSION_SECRET。
...
默认在 `http://localhost:8787` 打开应用。如需使用其他本地端口，请在启动 Docker Compose 前设置 `.env` 中的 `PORT`。公网部署和 Nginx 示例见 [部署文档](docs/deployment.zh-CN.md)。
...
`OPENAI_API_KEY` 可以在本地启动检查或使用 Codex 登录生成时留空。应用仍会启动；如果没有 API Key 也没有有效 Codex 会话，生成端点会返回 `missing_provider` JSON 错误，浏览器默认展示首页。
...
- `gpt-image-canvas.sqlite`：默认项目、生成历史、资产元数据、云端上传元数据、可选 COS 配置，以及 Codex OAuth token 记录。
- `assets/`：生成的图像文件。
...
- 缺少或空的 `OPENAI_API_KEY`：应用仍会启动。如果没有 Codex 会话，`/` 会显示首页，文生图和参考图请求会返回 `missing_provider`。可以把有效 key 添加到 `.env` 后重启 API 或 Docker 容器，也可以从首页使用 `Codex 登录`。
...
- Docker config 默认会输出 `.env` 值。真实凭证存在时，请使用 `docker compose config --quiet` 做验证，不要运行或分享普通 `docker compose config` 的完整展开输出。
