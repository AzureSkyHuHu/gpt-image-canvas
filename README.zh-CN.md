...
如果使用 API Key 生成，请在 `.env` 中设置 `OPENAI_API_KEY`。应用默认使用官方 OpenAI Image API 和 `gpt-image-2`。如需转发到 OpenAI 兼容端点，请在 `.env` 中设置 `OPENAI_BASE_URL`；如需使用不同的兼容图像模型，请设置 `OPENAI_IMAGE_MODEL`。

打开 `http://localhost:5173` 使用 Web 应用。

## 认证与路由

- `/` 是凭据感知首页。没有 `OPENAI_API_KEY` 且没有有效 Codex 会话时，会展示项目介绍，并提供 `Codex 登录` 和 `接入 API` 两个入口。
- `/canvas` 是画布工作区。没有生成服务时会回到 `/`；有 API Key 或有效 Codex 会话时，访问 `/` 会自动进入 `/canvas`。
- `/gallery` 在任何凭据状态下都可访问，方便查看本地已保存作品。
- Provider 优先级固定：非空 `OPENAI_API_KEY` 始终优先使用 OpenAI 兼容 Images API；只有没有 API Key 且 Codex 会话有效时才使用 Codex。
